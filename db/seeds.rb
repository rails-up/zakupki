DeliveryPaymentCostType.create([
  { value: 'по количеству людей', description: 'Общая сумма делится на количество людей' },
  { value: 'пропорционально стоимости заказа', description: 'Общая сумма делиться пропорционально сумме заказа каждого участника' },
  {value: 'пропорционально количеству наименований в заказе', description: 'Общая сумма делится пропорционально количеству наименований товаров в заказе'}
])

DeliveryPaymentType.create([
  { value: 'зависит от количества заказов', description: 'В данном случае за доставку пользователи платят при получении' },
  { value: 'фикисированная стоимость', description: 'В данном случае за доставку пользователи платят сразу' }
  ])

  City.create([
  { name: "Абаза" },
  { name: "Абакан" },
  { name: "Абан" },
  { name: "Абатский" },
  { name: "Абдулино" },
  { name: "Абезь" },
  { name: "Абинск" },
  { name: "Абрамцево" },
  { name: "Абрау-Дюрсо" },
  { name: "Абый" },
  { name: "Аган" },
  { name: "Агаповка" },
  { name: "Агвали" },
  { name: "Агеево" },
  { name: "Агинское" },
  { name: "Агрыз" },
  { name: "Адамовка" },
  { name: "Адыгейск" },
  { name: "Азнакаево" },
  { name: "Азов" },
  { name: "Айгунь" },
  { name: "Айдырлинский" },
  { name: "Айкино" },
  { name: "Ак-Довурак" },
  { name: "Акбулак" },
  { name: "Аккермановка" },
  { name: "Аксай" },
  { name: "Аксаково" },
  { name: "Аксарка" },
  { name: "Аксеново-Зиловское" },
  { name: "Аксубаево" },
  { name: "Акташ" },
  { name: "Актюбинский" },
  { name: "Акутиха" },
  { name: "Акуша" },
  { name: "Акша" },
  { name: "Алабино" },
  { name: "Алагир" },
  { name: "Алапаевск" },
  { name: "Алатырь" },
  { name: "Алдан" },
  { name: "Алейск" },
  { name: "Александро-Невский" },
  { name: "Александров Гай" },
  { name: "Александров" },
  { name: "Александровск-Сахалинский" },
  { name: "Александровская" },
  { name: "Александровский Завод" },
  { name: "Александровское" },
  { name: "Алексеевка" },
  { name: "Алексеевск" },
  { name: "Алексеевская" },
  { name: "Алексеевское" },
  { name: "Алексин" },
  { name: "Алзамай" },
  { name: "Аликово" },
  { name: "Алмазный" },
  { name: "Алнаши" },
  { name: "Алтайский" },
  { name: "Алтухово" },
  { name: "Алтынай" },
  { name: "Алущевск" },
  { name: "Алыгжер" },
  { name: "Альменево" },
  { name: "Альметьевск" },
  { name: "Амазар" },
  { name: "Амбарный" },
  { name: "Амга" },
  { name: "Амдерма" },
  { name: "Амзя" },
  { name: "Амурск" },
  { name: "Анадырь" },
  { name: "Анапа" },
  { name: "Анбэцу" },
  { name: "Ангарск" },
  { name: "Андреаполь" },
  { name: "Андреево" },
  { name: "Андропов" },
  { name: "Анжеро-Судженск" },
  { name: "Анива" },
  { name: "Анна" },
  { name: "Анопино" },
  { name: "Антропово" },
  { name: "Анучино" },
  { name: "Анциферово" },
  { name: "Апастово" },
  { name: "Апатиты" },
  { name: "Апрелевка" },
  { name: "Апшеронск" },
  { name: "Арамиль" },
  { name: "Арбагар" },
  { name: "Арбаж" },
  { name: "Аргаяш" },
  { name: "Аргун" },
  { name: "Ардатов" },
  { name: "Ардон" },
  { name: "Арзамас" },
  { name: "Арзгир" },
  { name: "Аркадак" },
  { name: "Аркуль" },
  { name: "Армавир" },
  { name: "Армизонское" },
  { name: "Аромашево" },
  { name: "Арсеньев" },
  { name: "Арсеньево" },
  { name: "Арск" },
  { name: "Артем" },
  { name: "Артемовск" },
  { name: "Артемовский" },
  { name: "Арти" },
  { name: "Архангельск" },
  { name: "Архангельское" },
  { name: "Архара" },
  { name: "Архипо-Осиповка" },
  { name: "Архиповка" },
  { name: "Аршань" },
  { name: "Арья" },
  { name: "Асбест" },
  { name: "Асекеево" },
  { name: "Асино" },
  { name: "Аскиз" },
  { name: "Аскино" },
  { name: "Астрахань" },
  { name: "Атагай" },
  { name: "Атамановка" },
  { name: "Атка" },
  { name: "Аткарск" },
  { name: "Атласово" },
  { name: "Атюрьево" },
  { name: "Атяшево" },
  { name: "Афипский" },
  { name: "Африканда" },
  { name: "Ахтубинск" },
  { name: "Ахты" },
  { name: "Ахтырский" },
  { name: "Ачинск" },
  { name: "Ачису" },
  { name: "Ачит" },
  { name: "Ачуево" },
  { name: "Аша" },
  { name: "Ашитково" },
  { name: "Аютинск" },
  { name: "Аян" },
  { name: "Аянка" },
  { name: "Бабаево" },
  { name: "Бабаюрт" },
  { name: "Бабушкин" },
  { name: "Бабынино" },
  { name: "Бавлены" },
  { name: "Бавлы" },
  { name: "Багаевский" },
  { name: "Баган" },
  { name: "Багдарин" },
  { name: "Багратионовск" },
  { name: "Баево" },
  { name: "Базарные Матаки" },
  { name: "Базарный Карабулак" },
  { name: "Базарный Сызган" },
  { name: "Бай Хаак" },
  { name: "Байкал" },
  { name: "Байкалово" },
  { name: "Байкальск" },
  { name: "Байкит" },
  { name: "Байконур" },
  { name: "Баймак" },
  { name: "Бакал" },
  { name: "Бакалы" },
  { name: "Баксан" },
  { name: "Бакчар" },
  { name: "Бакшеево" },
  { name: "Балабаново" },
  { name: "Балаганск" },
  { name: "Балакирево" },
  { name: "Балаково" },
  { name: "Балахна" },
  { name: "Балахта" },
  { name: "Балашиха" },
  { name: "Балашов" },
  { name: "Балезино" },
  { name: "Балей" },
  { name: "Балтай" },
  { name: "Балтаси" },
  { name: "Балтийск" },
  { name: "Балыгычан" },
  { name: "Балыкса" },
  { name: "Барабинск" },
  { name: "Баргузин" },
  { name: "Барда" },
  { name: "Барзас" },
  { name: "Барнаул" },
  { name: "Барсуки" },
  { name: "Барыбино" },
  { name: "Барыш" },
  { name: "Барятино" },
  { name: "Басьяновский" },
  { name: "Батагай" },
  { name: "Батайск" },
  { name: "Батецкий" },
  { name: "Батурино" },
  { name: "Батырева" },
  { name: "Башмаково" },
  { name: "Баянгол" },
  { name: "Баяндай" },
  { name: "Бегичевский" },
  { name: "Беднодемьяновск" },
  { name: "Бежаницы" },
  { name: "Бежецк" },
  { name: "Бежта" },
  { name: "Безенчук" },
  { name: "Беково" },
  { name: "Белая Березка" },
  { name: "Белая Калитва" },
  { name: "Белая Холуница" },
  { name: "Белая" },
  { name: "Белгород" },
  { name: "Белебей" },
  { name: "Белев" },
  { name: "Белинский" },
  { name: "Белово" },
  { name: "Белогорск" },
  { name: "Белозерск" },
  { name: "Белокуриха" },
  { name: "Беломорск" },
  { name: "Белоомут" },
  { name: "Белорецк" },
  { name: "Белореченск" },
  { name: "Белоусово" },
  { name: "Белоярск" },
  { name: "Белоярский" },
  { name: "Белые Берега" },
  { name: "Белые Столбы" },
  { name: "Белый Городок" },
  { name: "Белый Яр" },
  { name: "Белый" },
  { name: "Бельтырский" },
  { name: "Беляевка" },
  { name: "Бердигестях" },
  { name: "Бердск" },
  { name: "Бердюжье" },
  { name: "Береговой" },
  { name: "Березайка" },
  { name: "Березник" },
  { name: "Березники" },
  { name: "Березовский" },
  { name: "Березовый" },
  { name: "Берендеево" },
  { name: "Беринговский" },
  { name: "Беркакит" },
  { name: "Беслан" },
  { name: "Бессоновка" },
  { name: "Бестях" },
  { name: "Бетлица" },
  { name: "Бея" },
  { name: "Биаза" },
  { name: "Бижбуляк" },
  { name: "Бийск" },
  { name: "Бикин" },
  { name: "Билибино" },
  { name: "Бира" },
  { name: "Биракан" },
  { name: "Биробиджан" },
  { name: "Бирск" },
  { name: "Бирюсинск" },
  { name: "Бискамжа" },
  { name: "Бичура" },
  { name: "Благовещенск (Амурская обл.)" },
  { name: "Благовещенск" },
  { name: "Благодарный" },
  { name: "Бобров" },
  { name: "Богатое" },
  { name: "Богатые Сабы" },
  { name: "Богатырь" },
  { name: "Богданович" },
  { name: "Боговарово" },
  { name: "Боголюбово" },
  { name: "Богородицк" },
  { name: "Богородск" },
  { name: "Богородское" },
  { name: "Боготол" },
  { name: "Боград" },
  { name: "Богучаны" },
  { name: "Богучар" },
  { name: "Бодайбо" },
  { name: "Боковская" },
  { name: "Бокситогорск" },
  { name: "Бологое" },
  { name: "Болонь" },
  { name: "Болотное" },
  { name: "Болохово" },
  { name: "Болхов" },
  { name: "Большая Вишера" },
  { name: "Большая Глущица" },
  { name: "Большая Ижора" },
  { name: "Большая Мартыновка" },
  { name: "Большая Мурта" },
  { name: "Большая Речка" },
  { name: "Большая Соснова" },
  { name: "Большая Черниговка" },
  { name: "Большевик" },
  { name: "Большерецк" },
  { name: "Большереченск" },
  { name: "Большеречье" },
  { name: "Большеустьикинское" },
  { name: "Большие Березники" },
  { name: "Большие Уки" },
  { name: "Большое Болдино" },
  { name: "Большое Игнатово" },
  { name: "Большое Козино" },
  { name: "Большое Мурашкино" },
  { name: "Большое Нагаткино" },
  { name: "Большое Пикино" },
  { name: "Большое Полпино" },
  { name: "Большое Село" },
  { name: "Большое Солдатское" },
  { name: "Большое Сорокино" },
  { name: "Большой Камень" },
  { name: "Большой Луг" },
  { name: "Большой Улуй" },
  { name: "Бондари" },
  { name: "Бор" },
  { name: "Борзя" },
  { name: "Борисовка" },
  { name: "Борисоглебск" },
  { name: "Борисоглебский" },
  { name: "Боровичи" },
  { name: "Боровлянка" },
  { name: "Боровой" },
  { name: "Боровск" },
  { name: "Борогонцы" },
  { name: "Бородино" },
  { name: "Борское" },
  { name: "Ботлих" },
  { name: "Бохан" },
  { name: "Бошняково" },
  { name: "Братск" },
  { name: "Бреды" },
  { name: "Брежнев" },
  { name: "Брейтово" },
  { name: "Бронницы" },
  { name: "Брянск" },
  { name: "Бугульма" },
  { name: "Бугуруслан" },
  { name: "Буденновск" },
  { name: "Будогощь" },
  { name: "Бузулук" },
  { name: "Буинск" },
  { name: "Буй" },
  { name: "Буйнакск" },
  { name: "Букачача" },
  { name: "Буланаш" },
  { name: "Бураево" },
  { name: "Бурея" },
  { name: "Бурла" },
  { name: "Бурмакино" },
  { name: "Бурон" },
  { name: "Бурсоль" },
  { name: "Бутурлино" },
  { name: "Бутурлиновка" },
  { name: "Быков" },
  { name: "Быково" },
  { name: "Бытошь" },
  { name: "Вавож" },
  { name: "Вагай" },
  { name: "Вад" },
  { name: "Вадинск" },
  { name: "Валаам" },
  { name: "Валдай" },
  { name: "Валентин" },
  { name: "Валуево" },
  { name: "Валуйки" },
  { name: "Ванавара" },
  { name: "Ванино" },
  { name: "Ванкарем" },
  { name: "Варгаши" },
  { name: "Варегово" },
  { name: "Варна" },
  { name: "Варнавино" },
  { name: "Васильево" },
  { name: "Васильевский Мох" },
  { name: "Васильсурск" },
  { name: "Вахрушев" },
  { name: "Вахтан" },
  { name: "Вача" },
  { name: "Вачи" },
  { name: "Вейделевка" },
  { name: "Велегож" },
  { name: "Велетьма" },
  { name: "Велиж" },
  { name: "Великие Луки" },
  { name: "Великий Устюг" },
  { name: "Великодворский" },
  { name: "Вельск" },
  { name: "Венгерово" },
  { name: "Венев" },
  { name: "Вербилки" },
  { name: "Вербовский" },
  { name: "Верещагино" },
  { name: "Верея" },
  { name: "Верхнебаканский" },
  { name: "Верхневилюйск" },
  { name: "Верхнеднепровский" },
  { name: "Верхнеимбатск" },
  { name: "Верхнеколымск" },
  { name: "Верхнетуломский" },
  { name: "Верхнеуральск" },
  { name: "Верхнеяркеево" },
  { name: "Верхние Киги" },
  { name: "Верхние Татышлы" },
  { name: "Верхний Авзян" },
  { name: "Верхний Баскунчак" },
  { name: "Верхний Ландех" },
  { name: "Верхний Мамон" },
  { name: "Верхний Тагил" },
  { name: "Верхний Услон" },
  { name: "Верхний Уфалей" },
  { name: "Верхняя Инта" },
  { name: "Верхняя Пышма" },
  { name: "Верхняя Салда" },
  { name: "Верхняя Синячиха" },
  { name: "Верхняя Сысерть" },
  { name: "Верхняя Тойма" },
  { name: "Верхняя Тура" },
  { name: "Верхняя Хава" },
  { name: "Верховажье" },
  { name: "Верховье" },
  { name: "Верхозим" },
  { name: "Верхотурье" },
  { name: "Верхошижемье" },
  { name: "Верхоянск" },
  { name: "Ветлуга" },
  { name: "Вешенская" },
  { name: "Вешкайма" },
  { name: "Взморье" },
  { name: "Видим" },
  { name: "Видное" },
  { name: "Визинга" },
  { name: "Викулово" },
  { name: "Вилюйск" },
  { name: "Вилючинск" },
  { name: "Виля" },
  { name: "Винзили" },
  { name: "Вирандозеро" },
  { name: "Висим" },
  { name: "Витим" },
  { name: "Витимский" },
  { name: "Вихоревка" },
  { name: "Вичуга" },
  { name: "Владивосток" },
  { name: "Владикавказ" },
  { name: "Владимир" },
  { name: "Власово" },
  { name: "Внуково" },
  { name: "Водный" },
  { name: "Вожаель" },
  { name: "Вождь Пролетариата" },
  { name: "Вожега" },
  { name: "Возжаевка" },
  { name: "Вознесенское" },
  { name: "Вознесенье" },
  { name: "Возрождение" },
  { name: "Волга" },
  { name: "Волгоград" },
  { name: "Волгодонск" },
  { name: "Волгореченск" },
  { name: "Волжск" },
  { name: "Волжский" },
  { name: "Волово" },
  { name: "Вологда" },
  { name: "Володарск" },
  { name: "Володарский" },
  { name: "Волоколамск" },
  { name: "Волоконовка" },
  { name: "Волосово" },
  { name: "Волот" },
  { name: "Волочаевка Вторая" },
  { name: "Волошка" },
  { name: "Волхов" },
  { name: "Волчанск" },
  { name: "Волчиха" },
  { name: "Вольск" },
  { name: "Ворга" },
  { name: "Воркута" },
  { name: "Воробьевка" },
  { name: "Воронеж" },
  { name: "Вороново" },
  { name: "Воронцовка" },
  { name: "Воротынец" },
  { name: "Ворсма" },
  { name: "Воскресенск" },
  { name: "Воскресенское" },
  { name: "Восточный" },
  { name: "Востряково" },
  { name: "Восход" },
  { name: "Воткинск" },
  { name: "Вохтога" },
  { name: "Всеволожск" },
  { name: "Вуктыл" },
  { name: "Вурнары" },
  { name: "Выборг" },
  { name: "Выгоничи" },
  { name: "Выдрино" },
  { name: "Выездное" },
  { name: "Выкса" },
  { name: "Выползово" },
  { name: "Вырица" },
  { name: "Выселки" },
  { name: "Высокая Гора" },
  { name: "Высоковск" },
  { name: "Высокогорный" },
  { name: "Высокогорск" },
  { name: "Высоцк" },
  { name: "Вытегра" },
  { name: "Вычегодский" },
  { name: "Выша" },
  { name: "Вышков" },
  { name: "Вышний Волочек" },
  { name: "Вяземский" },
  { name: "Вязники" },
  { name: "Вязьма" },
  { name: "Вятские Поляны" },
  { name: "Гаврилов Посад" },
  { name: "Гаврилов Ям" },
  { name: "Гавриловка Вторая" },
  { name: "Гагарин" },
  { name: "Гагино" },
  { name: "Газимурский Завод" },
  { name: "Гай" },
  { name: "Гайны" },
  { name: "Галич" },
  { name: "Гари" },
  { name: "Гастелло" },
  { name: "Гатчина" },
  { name: "Гвардейск" },
  { name: "Гдов" },
  { name: "Геленджик" },
  { name: "Георгиевск" },
  { name: "Гергебиль" },
  { name: "Гешарт" },
  { name: "Гиагинская" },
  { name: "Гигант" },
  { name: "Гидроторф" },
  { name: "Гирвас" },
  { name: "Глазов" },
  { name: "Глазуновка" },
  { name: "Глинка" },
  { name: "Глотовка" },
  { name: "Глушково" },
  { name: "Глядянское" },
  { name: "Голицино" },
  { name: "Голынки" },
  { name: "Голышманово" },
  { name: "Горбатов" },
  { name: "Горбатовка" },
  { name: "Гордеевка" },
  { name: "Горелки" },
  { name: "Горно-Алтайск" },
  { name: "Горнозаводск" },
  { name: "Горные Ключи" },
  { name: "Горный" },
  { name: "Горняк" },
  { name: "Горняцкий" },
  { name: "Городец" },
  { name: "Городище" },
  { name: "Городищи" },
  { name: "Гороховец" },
  { name: "Горчуха" },
  { name: "Горшечное" },
  { name: "Горький" },
  { name: "Горьковское" },
  { name: "Горячегорск" },
  { name: "Горячий Ключ" },
  { name: "Грайворон" },
  { name: "Грамотеино" },
  { name: "Грахово" },
  { name: "Грачевка" },
  { name: "Гремячинск" },
  { name: "Грибановский" },
  { name: "Грозный" },
  { name: "Грязи" },
  { name: "Грязовец" },
  { name: "Губаха" },
  { name: "Губкин" },
  { name: "Губкинский" },
  { name: "Гудермез" },
  { name: "Гуково" },
  { name: "Гуниб" },
  { name: "Гурское" },
  { name: "Гурьевск" },
  { name: "Гусев" },
  { name: "Гусевский" },
  { name: "Гусиное Озеро" },
  { name: "Гусиноозерск" },
  { name: "Гусь Железный" },
  { name: "Гусь Хрустальный" },
  { name: "Давенда" },
  { name: "Давлеканово" },
  { name: "Давыдовка" },
  { name: "Дагестанские Огни" },
  { name: "Далматово" },
  { name: "Дальнегорск" },
  { name: "Дальнее Константиново" },
  { name: "Дальнереченск" },
  { name: "Данилов" },
  { name: "Данхов" },
  { name: "Дарасун" },
  { name: "Дарьинское" },
  { name: "Дебесы" },
  { name: "Дегтярск" },
  { name: "Деденево" },
  { name: "Дедовичи" },
  { name: "Дедовск" },
  { name: "Деманск" },
  { name: "Демидов" },
  { name: "Дербент" },
  { name: "Дербешкинский" },
  { name: "Дергачи" },
  { name: "Деревянка" },
  { name: "Деркул" },
  { name: "Десногорск" },
  { name: "Джамбейты" },
  { name: "Джаныбек" },
  { name: "Джержинский" },
  { name: "Джубга" },
  { name: "Дзержинск" },
  { name: "Дзержинское" },
  { name: "Дивеево" },
  { name: "Дивногорск" },
  { name: "Дивное" },
  { name: "Дигора" },
  { name: "Диксон" },
  { name: "Димитровград" },
  { name: "Динская" },
  { name: "Дмитриев-Льговский" },
  { name: "Дмитров" },
  { name: "Дмитровск-Орловский" },
  { name: "Дно" },
  { name: "Доброе" },
  { name: "Добрянка" },
  { name: "Довольное" },
  { name: "Долгое" },
  { name: "Долгопрудный" },
  { name: "Долгоруково" },
  { name: "Долинск" },
  { name: "Долматовский" },
  { name: "Домбай" },
  { name: "Домбаровский" },
  { name: "Домодедово" },
  { name: "Донецк" },
  { name: "Донское" },
  { name: "Донской" },
  { name: "Дормидонтовка" },
  { name: "Дорогобуж" },
  { name: "Дорохово" },
  { name: "Досчатое" },
  { name: "Дрезна" },
  { name: "Дровяная" },
  { name: "Дружная Горка" },
  { name: "Дубенский" },
  { name: "Дубки" },
  { name: "Дубна" },
  { name: "Дубовка" },
  { name: "Дубовский" },
  { name: "Дубовское" },
  { name: "Дубровка" },
  { name: "Дуван" },
  { name: "Дугна" },
  { name: "Дудинка" },
  { name: "Дудоровский" },
  { name: "Дульдурга" },
  { name: "Дуляпино" },
  { name: "Думиничи" },
  { name: "Духовницкое" },
  { name: "Духовщина" },
  { name: "Дылым" },
  { name: "Дюртюли" },
  { name: "Дятьково" },
  { name: "Евлашево" },
  { name: "Егорлыкская" },
  { name: "Егорьевск" },
  { name: "Ейск" },
  { name: "Екатеринбург" },
  { name: "Екатериновка" },
  { name: "Екатеринославка" },
  { name: "Екимовичи" },
  { name: "Елабуга" },
  { name: "Еланцы" },
  { name: "Елань" },
  { name: "Елань-Коленовский" },
  { name: "Елатьма" },
  { name: "Еленский" },
  { name: "Елец" },
  { name: "Елецкий" },
  { name: "Елизово" },
  { name: "Елово" },
  { name: "Ельники" },
  { name: "Ельня" },
  { name: "Ельцовка" },
  { name: "Еманжелинск" },
  { name: "Емва" },
  { name: "Емельяново" },
  { name: "Емца" },
  { name: "Енисейск" },
  { name: "Енотаевка" },
  { name: "Епифань" },
  { name: "Ербогачен" },
  { name: "Ермаковское" },
  { name: "Ермекеево" },
  { name: "Ермишь" },
  { name: "Ермолаево" },
  { name: "Ерофей Павлович" },
  { name: "Ертарский" },
  { name: "Ершичи" },
  { name: "Ершов" },
  { name: "Ершовка" },
  { name: "Ессентуки" },
  { name: "Ефимовский" },
  { name: "Ефремов" },
  { name: "Жарковский" },
  { name: "Железноводск" },
  { name: "Железногорск" },
  { name: "Железногорск" },
  { name: "Железногорск-Илимский" },
  { name: "Железнодорожный" },
  { name: "Жердевка" },
  { name: "Жигалово" },
  { name: "Жиганск" },
  { name: "Жигулевск" },
  { name: "Жиздра" },
  { name: "Жилево" },
  { name: "Жиндо" },
  { name: "Жирнов" },
  { name: "Жирновск" },
  { name: "Жирятино" },
  { name: "Жуковка" },
  { name: "Жуковский" },
  { name: "Забайкальск" },
  { name: "Забитуй" },
  { name: "Заветное" },
  { name: "Заветы Ильича" },
  { name: "Завитинск" },
  { name: "Заводопетровский" },
  { name: "Заводоуковск" },
  { name: "Заводоуспенское" },
  { name: "Заводской" },
  { name: "Заволжск" },
  { name: "Заволжье" },
  { name: "Завьялово" },
  { name: "Загорск" },
  { name: "Загорянский" },
  { name: "Задонск" },
  { name: "Заиграево" },
  { name: "Заинск" },
  { name: "Закаменск" },
  { name: "Залари" },
  { name: "Залегощь" },
  { name: "Залесово" },
  { name: "Заозерный" },
  { name: "Заозерск" },
  { name: "Заокский" },
  { name: "Западная Двина" },
  { name: "Заплюсье" },
  { name: "Заполярный" },
  { name: "Запрудная" },
  { name: "Зарайск" },
  { name: "Зареченск" },
  { name: "Заречный" },
  { name: "Заречье" },
  { name: "Заринск" },
  { name: "Зарубино" },
  { name: "Заток" },
  { name: "Захарово" },
  { name: "Зашеек" },
  { name: "Звездный" },
  { name: "Звенигово" },
  { name: "Звенигород" },
  { name: "Зверево" },
  { name: "Зебляки" },
  { name: "Зеленоборский" },
  { name: "Зеленогорск" },
  { name: "Зеленоград" },
  { name: "Зеленоградск" },
  { name: "Зеленодольск" },
  { name: "Зеленокумск" },
  { name: "Земетчино" },
  { name: "Зерноград" },
  { name: "Зея" },
  { name: "Зилаир" },
  { name: "Зима" },
  { name: "Зимовники" },
  { name: "Зирган" },
  { name: "Златоуст" },
  { name: "Златоустовск" },
  { name: "Злынка" },
  { name: "Змеиногорск" },
  { name: "Змиевка" },
  { name: "Знаменка" },
  { name: "Знаменск" },
  { name: "Знаменское" },
  { name: "Золотаревка" },
  { name: "Золотково" },
  { name: "Золотухино" },
  { name: "Зольное" },
  { name: "Зубова Поляна" },
  { name: "Зубцов" },
  { name: "Зуевка" },
  { name: "Зырянка" },
  { name: "Зыряновский" },
  { name: "Зырянское" },
  { name: "Зюзельский" },
  { name: "Зюкайка" },
  { name: "Ибреси" },
  { name: "Ивангород" },
  { name: "Иванищи" },
  { name: "Ивановка" },
  { name: "Иваново" },
  { name: "Ивантеевка" },
  { name: "Иваньковский" },
  { name: "Ивдель" },
  { name: "Ивня" },
  { name: "Иволгинск" },
  { name: "Ивот" },
  { name: "Игарка" },
  { name: "Иглино" },
  { name: "Игнатовка" },
  { name: "Игра" },
  { name: "Игрим" },
  { name: "Идель" },
  { name: "Идринское" },
  { name: "Идрица" },
  { name: "Ижевск" },
  { name: "Ижма" },
  { name: "Ижморский" },
  { name: "Избербаш" },
  { name: "Известковый" },
  { name: "Издешково" },
  { name: "Излучинск" },
  { name: "Измайлово" },
  { name: "Измалково" },
  { name: "Износки" },
  { name: "Изобильный" },
  { name: "Изоплит" },
  { name: "Изумруд" },
  { name: "Икряное" },
  { name: "Икша" },
  { name: "Иланский" },
  { name: "Илеза" },
  { name: "Илек" },
  { name: "Иловля" },
  { name: "Ильинский" },
  { name: "Ильинско-Подомское" },
  { name: "Ильинское-Хованское" },
  { name: "Илька" },
  { name: "Ильпырский" },
  { name: "Ильский" },
  { name: "Импалахти" },
  { name: "Инжавино" },
  { name: "Инза" },
  { name: "Инзер" },
  { name: "Иннокентьевка" },
  { name: "Иноземцево" },
  { name: "Инсар" },
  { name: "Инта" },
  { name: "Ипатово" },
  { name: "Ираель" },
  { name: "Ирбейское" },
  { name: "Ирбит" },
  { name: "Ириклинский" },
  { name: "Иркутск" },
  { name: "Ис" },
  { name: "Исаклы" },
  { name: "Исетское" },
  { name: "Исилькуль" },
  { name: "Искитим" },
  { name: "Исса" },
  { name: "Истра" },
  { name: "Исянгулово" },
  { name: "Итака" },
  { name: "Итатка" },
  { name: "Итатский" },
  { name: "Иульитин" },
  { name: "Ишеевка" },
  { name: "Ишим" },
  { name: "Ишимбай" },
  { name: "Йошкар-Ола" },
  { name: "Кабанск" },
  { name: "Кабардинка" },
  { name: "Кавалерово" },
  { name: "Кагальницкая" },
  { name: "Каджером" },
  { name: "Кадников" },
  { name: "Кадом" },
  { name: "Кадошкино" },
  { name: "Кадуй" },
  { name: "Кадый" },
  { name: "Кадыкчан" },
  { name: "Кажым" },
  { name: "Казановка" },
  { name: "Казанская" },
  { name: "Казанское" },
  { name: "Казань" },
  { name: "Казачинское" },
  { name: "Казинка" },
  { name: "Казталовка" },
  { name: "Казым-Мыс" },
  { name: "Калач" },
  { name: "Калач-на-Дону" },
  { name: "Калачинск" },
  { name: "Калашниково" },
  { name: "Калга" },
  { name: "Калевала" },
  { name: "Калинин" },
  { name: "Калининград" },
  { name: "Калинино" },
  { name: "Калининск" },
  { name: "Калининская" },
  { name: "Калмыково" },
  { name: "Калтан" },
  { name: "Калуга" },
  { name: "Калязин" },
  { name: "Кама" },
  { name: "Камбарка" },
  { name: "Каменка" },
  { name: "Каменногорск" },
  { name: "Каменномостский" },
  { name: "Каменоломни" },
  { name: "Каменск" },
  { name: "Каменск-Уральский" },
  { name: "Каменск-Шахтинский" },
  { name: "Каменский" },
  { name: "Каменское" },
  { name: "Камень-на-Оби" },
  { name: "Камень-Рыболов" },
  { name: "Камешково" },
  { name: "Каминский" },
  { name: "Камское Устье" },
  { name: "Камызяк" },
  { name: "Камышин" },
  { name: "Камышла" },
  { name: "Камышлов" },
  { name: "Канадей" },
  { name: "Кананикольское" },
  { name: "Канаш" },
  { name: "Кангалассы" },
  { name: "Кандалакша" },
  { name: "Кандры" },
  { name: "Каневская" },
  { name: "Канкунский" },
  { name: "Канск" },
  { name: "Кантемировка" },
  { name: "Капустин Яр" },
  { name: "Карабаново" },
  { name: "Карабаш" },
  { name: "Карабудахкент" },
  { name: "Карагай" },
  { name: "Караидель" },
  { name: "Караидельский" },
  { name: "Каракулино" },
  { name: "Карасук" },
  { name: "Карата" },
  { name: "Каратобе" },
  { name: "Каратузское" },
  { name: "Караул" },
  { name: "Карачаевск" },
  { name: "Карачев" },
  { name: "Каргаполье" },
  { name: "Каргасок" },
  { name: "Каргат" },
  { name: "Каргополь" },
  { name: "Кардымово" },
  { name: "Каринторф" },
  { name: "Карпинск" },
  { name: "Карпогоры" },
  { name: "Карпунинский" },
  { name: "Карсун" },
  { name: "Карталы" },
  { name: "Карымское" },
  { name: "Касимов" },
  { name: "Касли" },
  { name: "Каспийск" },
  { name: "Каспийский" },
  { name: "Касторное" },
  { name: "Касумкент" },
  { name: "Катав-Ивановск" },
  { name: "Катайга" },
  { name: "Катайск" },
  { name: "Катангли" },
  { name: "Катунки" },
  { name: "Качканар" },
  { name: "Качуг" },
  { name: "Кашары" },
  { name: "Кашин" },
  { name: "Кашира" },
  { name: "Квиток" },
  { name: "Кедровка" },
  { name: "Кежма" },
  { name: "Кез" },
  { name: "Кемерово" },
  { name: "Кемля" },
  { name: "Кемь" },
  { name: "Кенисберг" },
  { name: "Керва" },
  { name: "Керженец" },
  { name: "Керчевский" },
  { name: "Кесова Гора" },
  { name: "Кестеньга" },
  { name: "Кетово" },
  { name: "Кижинга" },
  { name: "Кизел" },
  { name: "Кизилюрт" },
  { name: "Кизляр" },
  { name: "Кизнер" },
  { name: "Кикерино" },
  { name: "Кикнур" },
  { name: "Килемары" },
  { name: "Кильдинстрой" },
  { name: "Кильмезь" },
  { name: "Кимовск" },
  { name: "Кимры" },
  { name: "Кингисепп" },
  { name: "Кинель" },
  { name: "Кинель-Черкасы" },
  { name: "Кинешма" },
  { name: "Киргиз-Мияки" },
  { name: "Киреевск" },
  { name: "Киренск" },
  { name: "Киржач" },
  { name: "Кириллов" },
  { name: "Кириши" },
  { name: "Киров" },
  { name: "Кировград" },
  { name: "Кирово-Чепецк" },
  { name: "Кировск" },
  { name: "Кировский" },
  { name: "Кирс" },
  { name: "Кирсанов" },
  { name: "Киря" },
  { name: "Киселевск" },
  { name: "Кисловодск" },
  { name: "Кичменгский Городок" },
  { name: "Киясово" },
  { name: "Клетня" },
  { name: "Клетский" },
  { name: "Климово" },
  { name: "Климовск" },
  { name: "Клин" },
  { name: "Клинцы" },
  { name: "Кличка" },
  { name: "Ключевский" },
  { name: "Ключи" },
  { name: "Клявлино" },
  { name: "Клязьма" },
  { name: "Княгинино" },
  { name: "Коболдо" },
  { name: "Кобра" },
  { name: "Кобринское" },
  { name: "Ковдор" },
  { name: "Ковернино" },
  { name: "Ковров" },
  { name: "Ковылкино" },
  { name: "Когалым" },
  { name: "Кодино" },
  { name: "Кодинск" },
  { name: "Кожевниково" },
  { name: "Кожино" },
  { name: "Кожым" },
  { name: "Козельск" },
  { name: "Козулька" },
  { name: "Козьмодемьянск" },
  { name: "Койгородок" },
  { name: "Кокаревка" },
  { name: "Кокошкино" },
  { name: "Коксовый" },
  { name: "Кокуй" },
  { name: "Кола" },
  { name: "Кологрив" },
  { name: "Коломна" },
  { name: "Колосовка" },
  { name: "Колпашево" },
  { name: "Колпино" },
  { name: "Колпны" },
  { name: "Колывань" },
  { name: "Колышлей" },
  { name: "Кольчугино" },
  { name: "Колюбакино" },
  { name: "Комаричи" },
  { name: "Коммунар" },
  { name: "Комсомольск" },
  { name: "Комсомольск-на-Амуре" },
  { name: "Комсомольский" },
  { name: "Комсомольское" },
  { name: "Конаково" },
  { name: "Конда" },
  { name: "Кондинское" },
  { name: "Кондоль" },
  { name: "Кондопога" },
  { name: "Кондрово" },
  { name: "Коноша" },
  { name: "Константиновск" },
  { name: "Конышевка" },
  { name: "Копейск" },
  { name: "Копьево" },
  { name: "Кораблино" },
  { name: "Коренево" },
  { name: "Кореновск" },
  { name: "Коркино" },
  { name: "Кормиловка" },
  { name: "Королев" },
  { name: "Короча" },
  { name: "Корсаков" },
  { name: "Корткерос" },
  { name: "Коряжма" },
  { name: "Коса" },
  { name: "Косино" },
  { name: "Кослан" },
  { name: "Костомукша" },
  { name: "Кострома" },
  { name: "Котельники" },
  { name: "Котельниково" },
  { name: "Котельнич" },
  { name: "Котлас" },
  { name: "Котово" },
  { name: "Котовск" },
  { name: "Кохма" },
  { name: "Кочево" },
  { name: "Кочкурово" },
  { name: "Кочубеевское" },
  { name: "Кочубей" },
  { name: "Кош-Агач" },
  { name: "Кошки" },
  { name: "Крапивинский" },
  { name: "Крапивная" },
  { name: "Красково" },
  { name: "Красная Гора" },
  { name: "Красная Горбатка" },
  { name: "Красная Заря" },
  { name: "Красноармейск" },
  { name: "Красноармейская" },
  { name: "Красноармейское" },
  { name: "Красноборск" },
  { name: "Красновишерск" },
  { name: "Красногвардейское" },
  { name: "Красногородское" },
  { name: "Красногорск" },
  { name: "Красногорское" },
  { name: "Краснодар" },
  { name: "Красное-на-Волге" },
  { name: "Краснозаводск" },
  { name: "Краснозерское" },
  { name: "Краснознаменск" },
  { name: "Краснокаменск" },
  { name: "Краснокамск" },
  { name: "Красноселькуп" },
  { name: "Краснослободск" },
  { name: "Краснотуранск" },
  { name: "Краснотурьинск" },
  { name: "Красноуральск" },
  { name: "Красноусольский" },
  { name: "Красноуфимск" },
  { name: "Краснощеково" },
  { name: "Красноярск" },
  { name: "Красные Баки" },
  { name: "Красные Четаи" },
  { name: "Красный Кут" },
  { name: "Красный Сулин" },
  { name: "Красный Ткач" },
  { name: "Красный Холм" },
  { name: "Красный Чикой" },
  { name: "Красный Яр" },
  { name: "Красный" },
  { name: "Кресцы" },
  { name: "Кривошеино" },
  { name: "Кромы" },
  { name: "Кронштадт" },
  { name: "Кропоткин" },
  { name: "Крутинка" },
  { name: "Крутиха" },
  { name: "Крыловская" },
  { name: "Крымск" },
  { name: "Крюково" },
  { name: "Кстово" },
  { name: "Кубинка" },
  { name: "Кувандык" },
  { name: "Кувшиново" },
  { name: "Кугеси" },
  { name: "Кудымкар" },
  { name: "Куеда" },
  { name: "Куженер" },
  { name: "Кузнецк" },
  { name: "Кузоватово" },
  { name: "Куйбышев" },
  { name: "Куйбышево" },
  { name: "Куйтун" },
  { name: "Кукмод" },
  { name: "Кукмор" },
  { name: "Кулебаки" },
  { name: "Култук" },
  { name: "Кулунда" },
  { name: "Кумены" },
  { name: "Кумертау" },
  { name: "Кумух" },
  { name: "Кумылженская" },
  { name: "Кунашак" },
  { name: "Кунгур" },
  { name: "Кунья" },
  { name: "Купавна" },
  { name: "Купино" },
  { name: "Курагино" },
  { name: "Курах" },
  { name: "Курган" },
  { name: "Курганинск" },
  { name: "Курильск" },
  { name: "Куркино" },
  { name: "Курманаевка" },
  { name: "Куровское" },
  { name: "Курсавка" },
  { name: "Курск" },
  { name: "Куртамыш" },
  { name: "Курумкан" },
  { name: "Курчатов" },
  { name: "Куса" },
  { name: "Кутулик" },
  { name: "Кушва" },
  { name: "Кушнаренково" },
  { name: "Кущевская" },
  { name: "Кшенский" },
  { name: "Кызыл" },
  { name: "Кыра" },
  { name: "Кырен" },
  { name: "Кытманово" },
  { name: "Кыштовка" },
  { name: "Кыштым" },
  { name: "Кяхта" },
  { name: "Кёнигсберг" },
  { name: "Лабинск" },
  { name: "Лабытнанги" },
  { name: "Лаврентия" },
  { name: "Лазо" },
  { name: "Лаишево" },
  { name: "Лангепас" },
  { name: "Ларьяк" },
  { name: "Лахденпохья" },
  { name: "Лебедянь" },
  { name: "Лебяжье" },
  { name: "Лев Толстой" },
  { name: "Левокумское" },
  { name: "Лениградская" },
  { name: "Лениногорск" },
  { name: "Ленинск" },
  { name: "Ленинск-Кузнецкий" },
  { name: "Ленинский" },
  { name: "Ленинское" },
  { name: "Ленск" },
  { name: "Лесное" },
  { name: "Лесной Городок" },
  { name: "Лесной" },
  { name: "Лесозаводск" },
  { name: "Лесосибирск" },
  { name: "Лешуконское" },
  { name: "Ливны" },
  { name: "Ликино-Дулево" },
  { name: "Лиман" },
  { name: "Липецк" },
  { name: "Липин Бор" },
  { name: "Лисий Нос" },
  { name: "Лиски" },
  { name: "Лихославль" },
  { name: "Лобня" },
  { name: "Лодейное Поле" },
  { name: "Локня" },
  { name: "Локоть" },
  { name: "Ломоносов" },
  { name: "Лопатино" },
  { name: "Лопатинский" },
  { name: "Лосино-Петровский" },
  { name: "Лотошино" },
  { name: "Лоухи" },
  { name: "Луга" },
  { name: "Луза" },
  { name: "Лукино" },
  { name: "Лукоянов" },
  { name: "Лух" },
  { name: "Луховицы" },
  { name: "Лучегорск" },
  { name: "Лысково" },
  { name: "Лысые Горы" },
  { name: "Лысьва" },
  { name: "Лыткарино" },
  { name: "Львовский" },
  { name: "Льгов" },
  { name: "Люберцы" },
  { name: "Любим" },
  { name: "Любинский" },
  { name: "Любытино" },
  { name: "Людиново" },
  { name: "Лямбирь" },
  { name: "Лянтор" },
  { name: "Магадан" },
  { name: "Магарамкент" },
  { name: "Магдагачи" },
  { name: "Магнитогорск" },
  { name: "Маджалис" },
  { name: "Майкоп" },
  { name: "Майна" },
  { name: "Майский" },
  { name: "Майя" },
  { name: "Макаров" },
  { name: "Макарьев" },
  { name: "Максатиха" },
  { name: "Макушино" },
  { name: "Малаховка" },
  { name: "Малая Вишера" },
  { name: "Малая Сердоба" },
  { name: "Малгобек" },
  { name: "Малмыж" },
  { name: "Малоархангельск" },
  { name: "Малояз" },
  { name: "Малоярославец" },
  { name: "Малые Дербеты" },
  { name: "Мама" },
  { name: "Мамадыш" },
  { name: "Мамоново" },
  { name: "Мамонтово" },
  { name: "Мантурово" },
  { name: "Марево" },
  { name: "Мари-Турек" },
  { name: "Мариинск" },
  { name: "Мариинский Посад" },
  { name: "Маркс" },
  { name: "Марьяновка" },
  { name: "Маслянино" },
  { name: "Матвеев Курган" },
  { name: "Матвеевка" },
  { name: "Махачкала" },
  { name: "Мглин" },
  { name: "Мегион" },
  { name: "Медведево" },
  { name: "Медвежьегорск" },
  { name: "Медвенка" },
  { name: "Медногорск" },
  { name: "Медынь" },
  { name: "Междуреченск" },
  { name: "Мезень" },
  { name: "Меленки" },
  { name: "Мелеуз" },
  { name: "Мельниково" },
  { name: "Менделеевск" },
  { name: "Мензелинск" },
  { name: "Менкеря" },
  { name: "Месягутово" },
  { name: "Мехельта" },
  { name: "Мещовск" },
  { name: "Миасс" },
  { name: "Мигулинская" },
  { name: "Миллерово" },
  { name: "Милославское" },
  { name: "Мильково" },
  { name: "Милютинская" },
  { name: "Минеральные Воды" },
  { name: "Минусинск" },
  { name: "Мирный" },
  { name: "Михайлов" },
  { name: "Михайловка" },
  { name: "Михайловск" },
  { name: "Михайловский" },
  { name: "Михайловское" },
  { name: "Михнево" },
  { name: "Мичуринск" },
  { name: "Мишкино" },
  { name: "Моготуй" },
  { name: "Могоча" },
  { name: "Можайск" },
  { name: "Можга" },
  { name: "Моздок" },
  { name: "Мокроус" },
  { name: "Мокроусово" },
  { name: "Мокшан" },
  { name: "Молоково" },
  { name: "Молчаново" },
  { name: "Монастырщина" },
  { name: "Монды" },
  { name: "Монино" },
  { name: "Мончегорск" },
  { name: "Моргауши" },
  { name: "Мордово" },
  { name: "Морки" },
  { name: "Морозовск" },
  { name: "Моршанск" },
  { name: "Мосальск" },
  { name: "Москва" },
  { name: "Мостовской" },
  { name: "Мотыгино" },
  { name: "Мошенское" },
  { name: "Мошково" },
  { name: "Мраково" },
  { name: "Муезерский" },
  { name: "Мужи" },
  { name: "Муравленко" },
  { name: "Мураши" },
  { name: "Мурманск" },
  { name: "Мурмаши" },
  { name: "Муром" },
  { name: "Муромцево" },
  { name: "Муслюмово" },
  { name: "Муханово" },
  { name: "Мухоршибирь" },
  { name: "Мучкапский" },
  { name: "Мценск" },
  { name: "Мыс Шмидта" },
  { name: "Мыски" },
  { name: "Мытищи" },
  { name: "Мышкино" },
  { name: "Набережные Челны" },
  { name: "Навашино" },
  { name: "Навля" },
  { name: "Нагорск" },
  { name: "Надым" },
  { name: "Назарово" },
  { name: "Назрань" },
  { name: "Называевск" },
  { name: "Нальчик" },
  { name: "Наровчат" },
  { name: "Нарофоминск" },
  { name: "Нарткала" },
  { name: "Нарышкино" },
  { name: "Нарьян-Мар" },
  { name: "Наурская" },
  { name: "Нахабино" },
  { name: "Находка" },
  { name: "Началово" },
  { name: "Невель" },
  { name: "Невельск" },
  { name: "Неверкино" },
  { name: "Невинномысск" },
  { name: "Невьянск" },
  { name: "Некрасовка" },
  { name: "Некрасовское" },
  { name: "Нелидово" },
  { name: "Нелькан" },
  { name: "Нема" },
  { name: "Неман" },
  { name: "Немчиновка" },
  { name: "Нерехта" },
  { name: "Нерчинск" },
  { name: "Нерчинский Завод" },
  { name: "Нерюнгри" },
  { name: "Нестеров" },
  { name: "Нефтегорск" },
  { name: "Нефтекамск" },
  { name: "Нефтекумск" },
  { name: "Нефтеюганск" },
  { name: "Нехаевский" },
  { name: "Нея" },
  { name: "Нижнеангарск" },
  { name: "Нижневартовск" },
  { name: "Нижнедевицк" },
  { name: "Нижнекамск" },
  { name: "Нижнеудинск" },
  { name: "Нижние Серги" },
  { name: "Нижний Ингаш" },
  { name: "Нижний Ломов" },
  { name: "Нижний Новгород" },
  { name: "Нижний Тагил" },
  { name: "Нижний Часучей" },
  { name: "Нижняя Омка" },
  { name: "Нижняя Салда" },
  { name: "Нижняя Тавда" },
  { name: "Нижняя Тура" },
  { name: "Никель" },
  { name: "Николаевск" },
  { name: "Николаевск-на-Амуре" },
  { name: "Никольск" },
  { name: "Никольское" },
  { name: "Новая Ляля" },
  { name: "Новая Малыкла" },
  { name: "Новая Усмань" },
  { name: "Новгород" },
  { name: "Новичиха" },
  { name: "Новоалександровск" },
  { name: "Новоалександровская" },
  { name: "Новоалтайск" },
  { name: "Новоаннинский" },
  { name: "Новобратцевский" },
  { name: "Новобурейский" },
  { name: "Нововаршавка" },
  { name: "Нововоронеж" },
  { name: "Нововятск" },
  { name: "Новодвинск" },
  { name: "Новодугино" },
  { name: "Новозыбков" },
  { name: "Новокузнецк" },
  { name: "Новокуйбышевск" },
  { name: "Новолакское" },
  { name: "Новомосковск" },
  { name: "Новониколаевский" },
  { name: "Новоорск" },
  { name: "Новопавловск" },
  { name: "Новоподрезково" },
  { name: "Новопокровка" },
  { name: "Новоржев" },
  { name: "Новороссийск" },
  { name: "Новоселицкое" },
  { name: "Новоселово" },
  { name: "Новосергиевка" },
  { name: "Новосибирск" },
  { name: "Новосиль" },
  { name: "Новосокольники" },
  { name: "Новоспасское" },
  { name: "Новотроицк" },
  { name: "Новоузенск" },
  { name: "Новоуральск" },
  { name: "Новохоперск" },
  { name: "Новочебоксарск" },
  { name: "Новочеркасск" },
  { name: "Новошахтинск" },
  { name: "Новошешминск" },
  { name: "Новые Бурасы" },
  { name: "Новый Некоуз" },
  { name: "Новый Оскол" },
  { name: "Новый Торьял" },
  { name: "Новый Уренгой" },
  { name: "Ногинск" },
  { name: "Ноглики" },
  { name: "Ножай-Юрт" },
  { name: "Нолинск" },
  { name: "Норильск" },
  { name: "Ноябрьск" },
  { name: "Нурлат" },
  { name: "Ныроб" },
  { name: "Нытва" },
  { name: "Нычалах" },
  { name: "Нюксеница" },
  { name: "Нюрба" },
  { name: "Нягань" },
  { name: "Няндома" },
  { name: "Обливская" },
  { name: "Облучье" },
  { name: "Обнинск" },
  { name: "Обоянь" },
  { name: "Обухово" },
  { name: "Объячево" },
  { name: "Одесское" },
  { name: "Одинцово" },
  { name: "Одоев" },
  { name: "Ожерелье" },
  { name: "Озерновский" },
  { name: "Озерск" },
  { name: "Озеры" },
  { name: "Озинки" },
  { name: "Оконешниково" },
  { name: "Октябрьск" },
  { name: "Октябрьский" },
  { name: "Октябрьское" },
  { name: "Окуловка" },
  { name: "Ола" },
  { name: "Олекминск" },
  { name: "Оленегорск" },
  { name: "Оленино" },
  { name: "Оловянная" },
  { name: "Олонец" },
  { name: "Ольга" },
  { name: "Ольховатка" },
  { name: "Ольховка" },
  { name: "Омонск" },
  { name: "Омск" },
  { name: "Омсукчан" },
  { name: "Омутинский" },
  { name: "Омутнинск" },
  { name: "Онгудай" },
  { name: "Онега" },
  { name: "Опалиха" },
  { name: "Опарино" },
  { name: "Опочка" },
  { name: "Орда" },
  { name: "Орджоникидзе" },
  { name: "Орджоникидзевская" },
  { name: "Ордынское" },
  { name: "Орел" },
  { name: "Оренбург" },
  { name: "Орехово-Зуево" },
  { name: "Оричи" },
  { name: "Орлик" },
  { name: "Орловский" },
  { name: "Орск" },
  { name: "Оршанка" },
  { name: "Оса" },
  { name: "Осинники" },
  { name: "Оссора" },
  { name: "Осташков" },
  { name: "Остров" },
  { name: "Островское" },
  { name: "Острогожск" },
  { name: "Отрадная" },
  { name: "Отрадный" },
  { name: "Оус" },
  { name: "Оха" },
  { name: "Оханск" },
  { name: "Охотск" },
  { name: "Очер" },
  { name: "Павино" },
  { name: "Павловка" },
  { name: "Павлово" },
  { name: "Павловск" },
  { name: "Павловская" },
  { name: "Павловский Посад" },
  { name: "Павлоградка" },
  { name: "Палана" },
  { name: "Палатка" },
  { name: "Палех" },
  { name: "Палкино" },
  { name: "Палласовка" },
  { name: "Панино" },
  { name: "Парабель" },
  { name: "Параньга" },
  { name: "Парголово" },
  { name: "Парень" },
  { name: "Партизанск" },
  { name: "Партизанское" },
  { name: "Парфентьево" },
  { name: "Парфино" },
  { name: "Пахачи" },
  { name: "Пачелма" },
  { name: "Певек" },
  { name: "Пенза" },
  { name: "Пено" },
  { name: "Первомайск" },
  { name: "Первомайский" },
  { name: "Первомайское" },
  { name: "Первоуральск" },
  { name: "Перевоз" },
  { name: "Переволоцкий" },
  { name: "Перелюб" },
  { name: "Переметное" },
  { name: "Перемышль" },
  { name: "Переславль-Залесский" },
  { name: "Переяславка" },
  { name: "Пермь" },
  { name: "Пески" },
  { name: "Пестово" },
  { name: "Пестравка" },
  { name: "Пестрецы" },
  { name: "Пестяки" },
  { name: "Песчанокопское" },
  { name: "Петровск" },
  { name: "Петровск-Забайкальский" },
  { name: "Петровское" },
  { name: "Петродворец" },
  { name: "Петрозаводск" },
  { name: "Петропавловка" },
  { name: "Петропавловск-Камчатский" },
  { name: "Петропавловское" },
  { name: "Петухово" },
  { name: "Петушки" },
  { name: "Печора" },
  { name: "Печоры" },
  { name: "Пижанка" },
  { name: "Пикалёво" },
  { name: "Пильна" },
  { name: "Пинега" },
  { name: "Пировское" },
  { name: "Пироговский" },
  { name: "Пителино" },
  { name: "Питерка" },
  { name: "Питкяранта" },
  { name: "Пичаево" },
  { name: "Плавск" },
  { name: "Пласт" },
  { name: "Плесецк" },
  { name: "Плюсса" },
  { name: "Повенец" },
  { name: "Поворино" },
  { name: "Погар" },
  { name: "Пограничный" },
  { name: "Подгоренский" },
  { name: "Подгорное" },
  { name: "Поддорье" },
  { name: "Подольск" },
  { name: "Подосиновец" },
  { name: "Подпорожье" },
  { name: "Покачи" },
  { name: "Покров" },
  { name: "Покровка" },
  { name: "Покровск" },
  { name: "Покровское" },
  { name: "Полевской" },
  { name: "Полесск" },
  { name: "Половинное" },
  { name: "Полтавка" },
  { name: "Полушкино" },
  { name: "Полярные Зори" },
  { name: "Полярный" },
  { name: "Поназырево" },
  { name: "Пономаревка" },
  { name: "Поныри" },
  { name: "Порецкое" },
  { name: "Поронайск" },
  { name: "Порхов" },
  { name: "Поспелиха" },
  { name: "Похвистнево" },
  { name: "Почеп" },
  { name: "Починки" },
  { name: "Починок" },
  { name: "Пошехонье-Володарск" },
  { name: "Поярково" },
  { name: "Правдинск" },
  { name: "Правдинский" },
  { name: "Преградная" },
  { name: "Приаргунск" },
  { name: "Привокзальный" },
  { name: "Приволжск" },
  { name: "Приволжье" },
  { name: "Приморско-Ахтарск" },
  { name: "Приобье" },
  { name: "Приозерск" },
  { name: "Пристень" },
  { name: "Приютное" },
  { name: "Провидения" },
  { name: "Прокопьевск" },
  { name: "Пролетарск" },
  { name: "Пролетарский" },
  { name: "Промышленная" },
  { name: "Пронск" },
  { name: "Протвино" },
  { name: "Прохладный" },
  { name: "Пряжа" },
  { name: "Прямицыно" },
  { name: "Псков" },
  { name: "Пугачев" },
  { name: "Пудож" },
  { name: "Пуровск" },
  { name: "Пустошка" },
  { name: "Путятино" },
  { name: "Пучеж" },
  { name: "Пушкин" },
  { name: "Пушкино" },
  { name: "Пушкинские Горы" },
  { name: "Пущино" },
  { name: "Пыталово" },
  { name: "Пыть-Ях" },
  { name: "Пышма" },
  { name: "Пятигорск" },
  { name: "Радищево" },
  { name: "Радовицкий" },
  { name: "Радужный" },
  { name: "Раевский" },
  { name: "Райчихинск" },
  { name: "Ракитное" },
  { name: "Раменское" },
  { name: "Рамешки" },
  { name: "Рамонь" },
  { name: "Рассказово" },
  { name: "Ребриха" },
  { name: "Ревда" },
  { name: "Реж" },
  { name: "Ремонтное" },
  { name: "Репьевка" },
  { name: "Реутов" },
  { name: "Решетниково" },
  { name: "Ржакса" },
  { name: "Ржаница" },
  { name: "Ржев" },
  { name: "Ровеньки" },
  { name: "Ровное" },
  { name: "Рогнедино" },
  { name: "Родино" },
  { name: "Родионово-Несветайская" },
  { name: "Родники" },
  { name: "Романовка" },
  { name: "Ромны" },
  { name: "Ромоданово" },
  { name: "Рославль" },
  { name: "Россошь" },
  { name: "Ростов" },
  { name: "Ростов-на-Дону" },
  { name: "Рошаль" },
  { name: "Ртищево" },
  { name: "Рублево" },
  { name: "Рубцовск" },
  { name: "Рудня" },
  { name: "Руза" },
  { name: "Рузаевка" },
  { name: "Русская Поляна" },
  { name: "Русский Камешкир" },
  { name: "Русский" },
  { name: "Рутул" },
  { name: "Рыбинск" },
  { name: "Рыбная Слобода" },
  { name: "Рыбное" },
  { name: "Рыльск" },
  { name: "Ряжск" },
  { name: "Рязань" },
  { name: "Савино" },
  { name: "Сайхин" },
  { name: "Салават" },
  { name: "Салехард" },
  { name: "Салтыковка" },
  { name: "Сальск" },
  { name: "Самагалтай" },
  { name: "Самара" },
  { name: "Самарга" },
  { name: "Самойловка" },
  { name: "Сангар" },
  { name: "Сандово" },
  { name: "Санкт-Петербург" },
  { name: "Санчурск" },
  { name: "Сапожок" },
  { name: "Сараи" },
  { name: "Саракташ" },
  { name: "Саранск" },
  { name: "Сарапул" },
  { name: "Саратов" },
  { name: "Саргатское" },
  { name: "Сарманово" },
  { name: "Саров" },
  { name: "Сарыг-Сеп" },
  { name: "Саскылах" },
  { name: "Сасово" },
  { name: "Сатка" },
  { name: "Сафакулево" },
  { name: "Сафоново" },
  { name: "Саяногорск" },
  { name: "Саянск" },
  { name: "Свердловск" },
  { name: "Светлогорск" },
  { name: "Светлоград" },
  { name: "Светлый Яр" },
  { name: "Светлый" },
  { name: "Свеча" },
  { name: "Свободный" },
  { name: "Себеж" },
  { name: "Северное" },
  { name: "Северный" },
  { name: "Северо-Енисейский" },
  { name: "Северо-Курильск" },
  { name: "Северобайкальск" },
  { name: "Северодвинск" },
  { name: "Североморск" },
  { name: "Североуральск" },
  { name: "Северск" },
  { name: "Северская" },
  { name: "Севск" },
  { name: "Сегежа" },
  { name: "Седельниково" },
  { name: "Сеймчан" },
  { name: "Селенгинск" },
  { name: "Селижарово" },
  { name: "Селты" },
  { name: "Семикаракорск" },
  { name: "Семилуки" },
  { name: "Сенгилей" },
  { name: "Серафимович" },
  { name: "Сергач" },
  { name: "Сергиев Посад" },
  { name: "Сердобск" },
  { name: "Серебряные Пруды" },
  { name: "Сернур" },
  { name: "Серов" },
  { name: "Серпухов" },
  { name: "Серышево" },
  { name: "Сестрорецк" },
  { name: "Сеченово" },
  { name: "Сибай" },
  { name: "Сим" },
  { name: "Синегорье" },
  { name: "Сковородино" },
  { name: "Скопин" },
  { name: "Славгород" },
  { name: "Славск" },
  { name: "Славянка" },
  { name: "Славянск-на-Кубани" },
  { name: "Сладково" },
  { name: "Сланцы" },
  { name: "Слободской" },
  { name: "Слюдянка" },
  { name: "Смидович" },
  { name: "Смирных" },
  { name: "Смоленск" },
  { name: "Смоленское" },
  { name: "Снежинск" },
  { name: "Снежногорск" },
  { name: "Собинка" },
  { name: "Советск" },
  { name: "Советская Гавань" },
  { name: "Советский" },
  { name: "Советское" },
  { name: "Сокол" },
  { name: "Сокольское" },
  { name: "Солигалич" },
  { name: "Соликамск" },
  { name: "Солнечногорск" },
  { name: "Солнечнодольск" },
  { name: "Солнцево" },
  { name: "Солонешное" },
  { name: "Солтон" },
  { name: "Соль-Илецк" },
  { name: "Сольвычегодск" },
  { name: "Сольцы" },
  { name: "Сонково" },
  { name: "Сорочинск" },
  { name: "Сортавала" },
  { name: "Сосково" },
  { name: "Сосново-Озерское" },
  { name: "Сосновоборск" },
  { name: "Сосновское" },
  { name: "Сосновый Бор" },
  { name: "Сосногорск" },
  { name: "Сосьва" },
  { name: "Софийск" },
  { name: "Софпорог" },
  { name: "Софрино" },
  { name: "Сочи" },
  { name: "Спас-Деменск" },
  { name: "Спас-Клепики" },
  { name: "Спасск-Дальний" },
  { name: "Спасск-Рязанский" },
  { name: "Спасское" },
  { name: "Спирово" },
  { name: "Среднеколымск" },
  { name: "Среднеуральск" },
  { name: "Средняя Ахтуба" },
  { name: "Сретенск" },
  { name: "Ставрополь" },
  { name: "Сталинград" },
  { name: "Старая Кулатка" },
  { name: "Старая Купавна" },
  { name: "Старая Майна" },
  { name: "Старая Полтавка" },
  { name: "Старая Русса" },
  { name: "Старбеево" },
  { name: "Старица" },
  { name: "Староаллейское" },
  { name: "Старобалтачево" },
  { name: "Стародуб" },
  { name: "Старое Дрожжаное" },
  { name: "Старое Шайгово" },
  { name: "Старожилово" },
  { name: "Староминская" },
  { name: "Старосубхангулово" },
  { name: "Старощербиновская" },
  { name: "Староюрьево" },
  { name: "Старый Оскол" },
  { name: "Степное" },
  { name: "Стерлибашево" },
  { name: "Стерлитамак" },
  { name: "Стойба" },
  { name: "Стрежевой" },
  { name: "Строитель" },
  { name: "Струги-Красные" },
  { name: "Ступино" },
  { name: "Суворов" },
  { name: "Суджа" },
  { name: "Судиславль" },
  { name: "Судогда" },
  { name: "Суздаль" },
  { name: "Суземка" },
  { name: "Сузун" },
  { name: "Суксун" },
  { name: "Суна" },
  { name: "Сунтар" },
  { name: "Суоярви" },
  { name: "Сура" },
  { name: "Сураж" },
  { name: "Сургут (Самарская обл.)" },
  { name: "Сургут" },
  { name: "Суровикино" },
  { name: "Сурское" },
  { name: "Сусанино" },
  { name: "Сусуман" },
  { name: "Суть-Холь" },
  { name: "Сухиничи" },
  { name: "Сухой Лог" },
  { name: "Сходня" },
  { name: "Сызрань" },
  { name: "Сыктывкар" },
  { name: "Сысерть" },
  { name: "Сычевка" },
  { name: "Сюмси" },
  { name: "Сямжа" },
  { name: "Таборы" },
  { name: "Табуны" },
  { name: "Тавда" },
  { name: "Таврическое" },
  { name: "Таганрог" },
  { name: "Тазовский" },
  { name: "Тайга" },
  { name: "Тайшет" },
  { name: "Таксимо" },
  { name: "Талдом" },
  { name: "Талица" },
  { name: "Таловая" },
  { name: "Тальменка" },
  { name: "Тамала" },
  { name: "Тамбов" },
  { name: "Тамбовка" },
  { name: "Тара" },
  { name: "Тарасовский" },
  { name: "Тарко" },
  { name: "Тарко-Сале" },
  { name: "Тарногский Городок" },
  { name: "Тарумовка" },
  { name: "Таруса" },
  { name: "Тасеево" },
  { name: "Татарск" },
  { name: "Татищево" },
  { name: "Тацинский" },
  { name: "Ташла" },
  { name: "Таштагол" },
  { name: "Таштып" },
  { name: "Тбилисская" },
  { name: "Тверь" },
  { name: "Теберда" },
  { name: "Тевриз" },
  { name: "Тейково" },
  { name: "Текстильщик" },
  { name: "Темников" },
  { name: "Темпы" },
  { name: "Темрюк" },
  { name: "Теньгушево" },
  { name: "Тербуны" },
  { name: "Терек" },
  { name: "Терекли-Мектеб" },
  { name: "Тереньга" },
  { name: "Терней" },
  { name: "Терновка" },
  { name: "Тетюши" },
  { name: "Тигиль" },
  { name: "Тикси" },
  { name: "Тиличики" },
  { name: "Тим" },
  { name: "Тимашевск" },
  { name: "Тисуль" },
  { name: "Тихвин" },
  { name: "Тихорецк" },
  { name: "Тишково" },
  { name: "Тлярата" },
  { name: "Тобольск" },
  { name: "Тогучин" },
  { name: "Токаревка" },
  { name: "Тольятти" },
  { name: "Томари" },
  { name: "Томилино" },
  { name: "Томск" },
  { name: "Тонкино" },
  { name: "Тоншаево" },
  { name: "Тоора-Хем" },
  { name: "Топки" },
  { name: "Топчиха" },
  { name: "Торбеево" },
  { name: "Торжок" },
  { name: "Торопец" },
  { name: "Тосно" },
  { name: "Тотьма" },
  { name: "Тоцкое" },
  { name: "Тпиг" },
  { name: "Трехгорный" },
  { name: "Троицк" },
  { name: "Троицко-Печерск" },
  { name: "Троицкое" },
  { name: "Тросна" },
  { name: "Трубчевск" },
  { name: "Туапсе" },
  { name: "Туголесский Бор" },
  { name: "Тугулым" },
  { name: "Тугур" },
  { name: "Тужа" },
  { name: "Туймазы" },
  { name: "Тула" },
  { name: "Тулун" },
  { name: "Тульский" },
  { name: "Тупик" },
  { name: "Тура" },
  { name: "Туран" },
  { name: "Туринск" },
  { name: "Туринская Слобода" },
  { name: "Турки" },
  { name: "Турочак" },
  { name: "Турунтаево" },
  { name: "Туруханск" },
  { name: "Тутаев" },
  { name: "Тучково" },
  { name: "Тымовское" },
  { name: "Тымск" },
  { name: "Тында" },
  { name: "Тырныауз" },
  { name: "Тээли" },
  { name: "Тюкалинск" },
  { name: "Тюльган" },
  { name: "Тюменцево" },
  { name: "Тюмень" },
  { name: "Тюхтет" },
  { name: "Тяжинский" },
  { name: "Убинское" },
  { name: "Ува" },
  { name: "Уваровка" },
  { name: "Уварово" },
  { name: "Уват" },
  { name: "Увельский" },
  { name: "Углегорск" },
  { name: "Углич" },
  { name: "Угловское" },
  { name: "Угра" },
  { name: "Удельная" },
  { name: "Удомля" },
  { name: "Ужур" },
  { name: "Узловая" },
  { name: "Уинское" },
  { name: "Уйское" },
  { name: "Улан-Удэ" },
  { name: "Улеты" },
  { name: "Ульяново" },
  { name: "Ульяновск" },
  { name: "Умба" },
  { name: "Умет" },
  { name: "Унеча" },
  { name: "Уни" },
  { name: "Унъюган" },
  { name: "Упорово" },
  { name: "Уразовка" },
  { name: "Урай" },
  { name: "Уральск" },
  { name: "Урень" },
  { name: "Уржум" },
  { name: "Уркарах" },
  { name: "Урмары" },
  { name: "Урус-Мартан" },
  { name: "Урюпинск" },
  { name: "Усвяты" },
  { name: "Усинск" },
  { name: "Усмань" },
  { name: "Усогорск" },
  { name: "Усолье" },
  { name: "Усолье-Сибирское" },
  { name: "Успенское" },
  { name: "Уссурийск" },
  { name: "Устинов" },
  { name: "Усть-Авам" },
  { name: "Усть-Белая" },
  { name: "Усть-Большерецк" },
  { name: "Усть-Джегута" },
  { name: "Усть-Донецкий" },
  { name: "Усть-Илимск" },
  { name: "Усть-Ишим" },
  { name: "Усть-Калманка" },
  { name: "Усть-Камчатск" },
  { name: "Усть-Кан" },
  { name: "Усть-Катав" },
  { name: "Усть-Кишерть" },
  { name: "Усть-Кокса" },
  { name: "Усть-Кулом" },
  { name: "Усть-Кут" },
  { name: "Усть-Лабинск" },
  { name: "Усть-Мая" },
  { name: "Усть-Нера" },
  { name: "Усть-Омчуг" },
  { name: "Усть-Ордынский" },
  { name: "Усть-Тарка" },
  { name: "Усть-Уда" },
  { name: "Усть-Улаган" },
  { name: "Усть-Цильма" },
  { name: "Усть-Чарышская Пристань" },
  { name: "Устюжна" },
  { name: "Утта" },
  { name: "Уфа" },
  { name: "Ухолово" },
  { name: "Ухта" },
  { name: "Учалы" },
  { name: "Уяр" },
  { name: "Фаленки" },
  { name: "Фатеж" },
  { name: "Федоровка" },
  { name: "Ферзиково" },
  { name: "Фершампенуаз" },
  { name: "Фирово" },
  { name: "Фирсановка" },
  { name: "Фокино" },
  { name: "Фосфоритный" },
  { name: "Фролово" },
  { name: "Фрязино" },
  { name: "Фряново" },
  { name: "Фурманов" },
  { name: "Фурманово" },
  { name: "Хабаровск" },
  { name: "Хабары" },
  { name: "Хабез" },
  { name: "Халтурин" },
  { name: "Хандыга" },
  { name: "Ханты-Мансийск" },
  { name: "Харабали" },
  { name: "Харовск" },
  { name: "Хасавюрт" },
  { name: "Хасан" },
  { name: "Хатанга" },
  { name: "Хвалынск" },
  { name: "Хвастовичи" },
  { name: "Хвойное" },
  { name: "Хворостянка" },
  { name: "Хив" },
  { name: "Хилок" },
  { name: "Химки" },
  { name: "Хиславичи" },
  { name: "Хлевное" },
  { name: "Хову-Аксы" },
  { name: "Холм" },
  { name: "Холм-Жирковский" },
  { name: "Холмогоры" },
  { name: "Холмск" },
  { name: "Хомутовка" },
  { name: "Хомутово" },
  { name: "Хонуу" },
  { name: "Хоринск" },
  { name: "Хорлово" },
  { name: "Хороль" },
  { name: "Хотынец" },
  { name: "Хотьково" },
  { name: "Хохольский" },
  { name: "Хунзах" },
  { name: "Цаган-Аман" },
  { name: "Целина" },
  { name: "Целинное" },
  { name: "Цивильск" },
  { name: "Цимлянск" },
  { name: "Цуриб" },
  { name: "Чагода" },
  { name: "Чадан" },
  { name: "Чайковский" },
  { name: "Чалтырь" },
  { name: "Чамзинка" },
  { name: "Чаны" },
  { name: "Чапаев" },
  { name: "Чапаевск" },
  { name: "Чаплыгин" },
  { name: "Чара" },
  { name: "Чарышское" },
  { name: "Частые" },
  { name: "Чебаркуль" },
  { name: "Чебоксары" },
  { name: "Чегдомын" },
  { name: "Чегем-Первый" },
  { name: "Чекмагуш" },
  { name: "Челно-Вершины" },
  { name: "Челябинск" },
  { name: "Чердаклы" },
  { name: "Чердынь" },
  { name: "Черемисиново" },
  { name: "Черемушки" },
  { name: "Черемхово" },
  { name: "Черепаново" },
  { name: "Череповец" },
  { name: "Черкесск" },
  { name: "Черкизово" },
  { name: "Черлак" },
  { name: "Черниговка" },
  { name: "Черноголовка" },
  { name: "Черногорск" },
  { name: "Чернореченский" },
  { name: "Чернушка" },
  { name: "Черный Яр" },
  { name: "Чернышевск" },
  { name: "Чернышковский" },
  { name: "Чернянка" },
  { name: "Черняховск" },
  { name: "Черский" },
  { name: "Чертково" },
  { name: "Черусти" },
  { name: "Чесма" },
  { name: "Чехов" },
  { name: "Чикола" },
  { name: "Чистоозерное" },
  { name: "Чистополь" },
  { name: "Чита" },
  { name: "Чишмы" },
  { name: "Чкаловск" },
  { name: "Чокурдах" },
  { name: "Чугуевка" },
  { name: "Чудово" },
  { name: "Чулым" },
  { name: "Чумикан" },
  { name: "Чунский" },
  { name: "Чурапча" },
  { name: "Чусовой" },
  { name: "Чухлома" },
  { name: "Чучково" },
  { name: "Шаблыкино" },
  { name: "Шагонар" },
  { name: "Шадринск" },
  { name: "Шали" },
  { name: "Шалинское" },
  { name: "Шаран" },
  { name: "Шаранга" },
  { name: "Шарапово" },
  { name: "Шаркан" },
  { name: "Шарлык" },
  { name: "Шарыпово" },
  { name: "Шарья" },
  { name: "Шатки" },
  { name: "Шатрово" },
  { name: "Шатура" },
  { name: "Шатурторф" },
  { name: "Шаховская" },
  { name: "Шахтерск" },
  { name: "Шахты" },
  { name: "Шахунья" },
  { name: "Шацк" },
  { name: "Шебалино" },
  { name: "Шебекино" },
  { name: "Шексна" },
  { name: "Шелаболиха" },
  { name: "Шелехов" },
  { name: "Шелопугино" },
  { name: "Шемурша" },
  { name: "Шемышейка" },
  { name: "Шемятино" },
  { name: "Шенкурск" },
  { name: "Шентала" },
  { name: "Шербакуль" },
  { name: "Шереметьевский" },
  { name: "Шигоны" },
  { name: "Шилка" },
  { name: "Шилово" },
  { name: "Шимановск" },
  { name: "Шимск" },
  { name: "Шипуново" },
  { name: "Шира" },
  { name: "Шлиссельбург" },
  { name: "Шовгеновский" },
  { name: "Шолоховский" },
  { name: "Шуйское" },
  { name: "Шумерля" },
  { name: "Шумиха" },
  { name: "Шумячи" },
  { name: "Шушенское" },
  { name: "Шуя" },
  { name: "Щекино" },
  { name: "Щелково" },
  { name: "Щербинка" },
  { name: "Щигры" },
  { name: "Щучье" },
  { name: "Эвенск" },
  { name: "Эгвекинот" },
  { name: "Экимчан" },
  { name: "Электрогорск" },
  { name: "Электросталь" },
  { name: "Электроугли" },
  { name: "Элиста" },
  { name: "Энгельс" },
  { name: "Энергетик" },
  { name: "Эрзин" },
  { name: "Эртиль" },
  { name: "Югорск" },
  { name: "Южа" },
  { name: "Южно-Курильск" },
  { name: "Южно-Сахалинск" },
  { name: "Южно-Сухокумск" },
  { name: "Южно-Уральск" },
  { name: "Юкаменское" },
  { name: "Юрга" },
  { name: "Юргамыш" },
  { name: "Юрибей" },
  { name: "Юрино" },
  { name: "Юрла" },
  { name: "Юрьев-Польский" },
  { name: "Юрьевец" },
  { name: "Юрья" },
  { name: "Юрюзань" },
  { name: "Юста" },
  { name: "Юсьва" },
  { name: "Юхнов" },
  { name: "Ягодное" },
  { name: "Ядрин" },
  { name: "Ядрино" },
  { name: "Яковлевка" },
  { name: "Якутск" },
  { name: "Якшур-Бодья" },
  { name: "Ялуторовск" },
  { name: "Яльчики" },
  { name: "Янаул" },
  { name: "Янтиково" },
  { name: "Яр" },
  { name: "Яр-Сале" },
  { name: "Яранск" },
  { name: "Яренск" },
  { name: "Ярково" },
  { name: "Ярославль" },
  { name: "Ярцево" },
  { name: "Ясногорск" },
  { name: "Ясный" },
  { name: "Яхрома" },
  { name: "Яшалта" },
  { name: "Яшкино" },
  { name: "Яшкуль" },
  { name: "Яя" }
])
